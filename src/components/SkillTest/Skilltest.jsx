...
function SkillTest() {
  ...
  //   try {
  //     const response = await axios.get(`https://api.novajobs.us/api/students/user-skills?skill_id=${id}&skill_name=${name}`, {
  //       headers: {
  //         Authorization: token,
  ...
  const getSkillTestQuestion = (id, name) => {
    setLoader(true);
    axios.get(`https://api.novajobs.us/api/students/user-skills?skill_id=${id}&skill_name=${name}`, {
      headers: {
        Authorization: token,
        "Content-type": "application/json",
      },
    ...
    .then((response) => {
      setLoader(false);
      // Assuming questions are returned in the response and we just pass the ID and Name to the next page
      router.push({
        pathname: '/dashboard/skill-test-question',
        query: { skillId: id, skillName: name },
      });
    })
    .catch((err) => {
      console.error("Error fetching skill test question:", err);
  ...
  useEffect(() => {
    if (token) {
      axios.get("https://api.novajobs.us/api/students/user-skills", {
        headers: {
          Authorization: token,
          "Content-type": "application/json",
        },
